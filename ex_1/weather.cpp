...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  #pragma acc data copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS], tend[0:nx*nz*NUM_VARS], flux[0:(nx+1)*(nz+1)*NUM_VARS], stencil, vals, d3_vals)
  {
    //Compute fluxes in the x-direction for each cell
    #pragma acc parallel loop collapse(4) private(r, u, w, t, p, inds)
    for (k=0; k<nz; k++) {
      for (i=0; i<nx+1; i++) {
...
void set_halo_values_x( double *state ) {
  ...
  #pragma acc parallel loop collapse(3) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS], recvbuf_l[0:hs*nz*NUM_VARS], recvbuf_r[0:hs*nz*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      #pragma acc parallel loop collapse(2) private(z, ind_r, ind_u, ind_t) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
      for (k=0; k<nz; k++) {
        for (i=0; i<hs; i++) {
          z = (k_beg + k+0.5)*dz;
...
void set_halo_values_z( double *state ) {
  ...
  #pragma acc parallel loop collapse(2) private(x, xloc, mnt_deriv) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
